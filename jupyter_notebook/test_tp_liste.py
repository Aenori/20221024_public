    ...
    def test_supprimer_troisieme_element(self):
        ...
        for it in range(10):
            try:
                ...
                self.Fail( "Votre fonction a provoqué une erreur lorsqu'elle a été appelée avec {0} : erreur {1}".format(prev, e) )

class TestExercice1a( unittest.TestCase ):
    def test_code(self):
        self.assertFalse( "range" in In[-1], "Vous ne devez pas utiliser de range" )
        self.assertEqual( In[-1].count("for "), 1, "Vous devez utiliser exactement une boucle for, vous en avez utilisées {0}".format(In[-1].count("for ")) )
